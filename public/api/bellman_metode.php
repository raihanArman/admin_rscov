<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'GET'){
        ...
        $queryTempJarak = "TRUNCATE TABLE tb_temp_jarak_bell";
                    ...
                    while($dataAsal = mysqli_fetch_array($sqlTempAlamatAsal)){
                        ...
                        if($sqlTempAlamatTujuan){
                            while($dataTujuan = mysqli_fetch_array($sqlTempAlamatTujuan)){
                                ...
                                $nilaiNode = 0;
                                $query = mysqli_query($con, "SELECT * FROM tb_temp_jarak_bell WHERE id_asal = $dataAsal[id_lokasi] AND id_tujuan = $dataTujuan[id_lokasi]-1");
                                ...
                                if($query){
                                    ...
                                    $rumus = $nilaiNode+$jarak;
                                    
                                    $queryInsert = "INSERT INTO tb_temp_jarak_bell(id_asal, id_tujuan, jarak, nilai) VALUES('$dataAsal[id_lokasi]', $dataTujuan[id_lokasi], '$jarak', '$rumus')";
        ...
        if($sql){
            while($row = mysqli_fetch_array($sql)){
                $queryDelete = "DELETE FROM tb_temp_jarak_bell WHERE id_asal = $row[id_lokasi] AND id_tujuan = $row[id_lokasi]";
        ...
        if($sql){
            ...
            for($a = 1; $a<=$jumlah; $a++){
                ...
                if($sqlAll){
                    ...
                    $queryMinData = "SELECT * FROM tb_temp_jarak_bell WHERE id_asal = $titik";
                    ...
                    if($sqlMinData){
                        ...
                        while($row = mysqli_fetch_array($sqlAll)){
                            if($titik != $row['id_lokasi']){
                                $queryMin = "SELECT * FROM tb_temp_jarak_bell WHERE id_asal = $titik AND id_tujuan = $row[id_lokasi]";
                        ...
                        $queryDeleteTitikX = "DELETE FROM tb_temp_jarak_bell WHERE id_tujuan = $titik";
